...
import { updateTaskMutationFn } from "@/lib/api";  // Update function
import { useMutation, useQueryClient } from "@tanstack/react-query";
import { toast } from "@/hooks/use-toast";

export default function UpdateTaskForm(props: {
  task?: any; // Task object to update
  projectId?: string;
  onClose: () => void;
}) {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: updateTaskMutationFn, // The update mutation function
  });
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      priority: task?.priority || TaskPriorityEnum.LOW,
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    ...
    const payload = {
      workspaceId,
      taskId: task?._id, // Use the task ID for update
      projectId: values.projectId, // Add projectId to the payload
      data: {
        ...values,
